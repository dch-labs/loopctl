...
- `SessionConfig::compact_threshold` now clamps into its documented `0..=100` range on every construction path. Previously only the `with_compact_threshold` builder clamped; `Default`, `Deserialize`, and round-tripped configs could carry an out-of-range value (e.g. `200` from disk), which the compaction subsystem would then interpret as "never compact." A single canonical `validate` point plus a field-level serde deserialize helper now enforce the range silently. Non-breaking (silent clamp; no signature changes).
