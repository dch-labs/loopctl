...
- `LoopMachine::compaction_noop(tokens_after)` — a second compaction feed method for passes that changed nothing (no compactor ran, a pre-compact hook vetoed, or the compactor returned the conversation unchanged). Unlike `compaction_result`, it leaves the committed history and the pending buffer untouched and adopts the driver's measured estimate as the current context size, applying the same no-progress guard (terminal `ContextExceeded` when nothing was shaved off). Alt drivers servicing `MachineStep::Compact` should use it for no-op passes.
