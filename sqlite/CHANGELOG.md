...
- **`access_count` decodes saturate instead of reading as an honest zero** — a foreign writer's absurd count (negative, or past `usize` on narrow targets) now saturates on the way out as documented, instead of silently loading as zero; pinned by `decode_entry_saturates_an_absurd_access_count`. Docs in the same pass: the crate's durability claim now draws the process-crash vs power-loss line (`NORMAL` sync may lose the most recent commits to power loss), and the duplicate-id behavior — `INSERT OR REPLACE` keeps the newest single copy where the flat backends keep both lines — is stated on the write path and pinned by `duplicate_id_stores_collapse_to_one_copy_here_but_not_in_the_file_store`.
