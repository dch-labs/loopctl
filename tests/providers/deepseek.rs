//! Cassette replay for the DeepSeek client.
//!
...
//! by one byte from what the real DeepSeek API accepted when the
...
async fn minimal_text_replays_from_cassette() {
    let scenario = cassette::scenarios()
        .into_iter()
        .find(|s| s.provider == "deepseek" && s.name == "minimal_text")
        .expect("the deepseek minimal_text scenario exists");
