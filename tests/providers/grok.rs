//! Cassette replay for the Grok client.
//!
...
//! by one byte from what the real Grok API accepted when the
...
async fn minimal_text_replays_from_cassette() {
    let scenario = cassette::scenarios()
        .into_iter()
        .find(|s| s.provider == "grok" && s.name == "minimal_text")
        .expect("the grok minimal_text scenario exists");
